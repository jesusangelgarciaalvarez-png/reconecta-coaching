...
document.addEventListener('DOMContentLoaded', () => {
    ...
    console.log('--- COACHPRO AI DEMO ENGINE v1.0 ---');
    ...
    const elements = {
        ...
        formSection: document.getElementById('step-2'),
    ...
    elements.confirmBtn.onclick = async () => {
        ...
        if (!name || !email || !phone || !selectedDate || !selectedTime) {
            elements.errorMsg.textContent = "Please complete all fields and select a date and time.";
        ...
        try {
            ...
            elements.confirmBtn.disabled = true;
            elements.confirmBtn.textContent = 'Calculating Rate...';
            ...
            try {
                ...
                const response = await fetch('/api/save', {
                    ...
                    body: JSON.stringify({ ...checkData, previewOnly: true, isDemo: true })
                });
                ...
                elements.errorMsg.textContent = 'Server connection failed. Please try again.';
                elements.errorMsg.classList.remove('hidden');
            } finally {
                elements.confirmBtn.disabled = false;
                elements.confirmBtn.textContent = 'Book Session';
            }
        } catch (err) {
        ...
        elements.payBtn.onclick = async () => {
            ...
            try {
                const response = await fetch('/api/save', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ ...finalData, isDemo: true })
                });

                if (response.ok) {
                    const result = await response.json();
                    sessionStorage.setItem('lastBooking', JSON.stringify({ ...finalData, isFree: result.finalPrice === 0 }));
                    window.location.href = `/demo-success.html?id=${appointmentId}&name=${encodeURIComponent(finalData.name)}&date=${finalData.date}&time=${finalData.time}&msg=${encodeURIComponent('Payment confirmed')}`;
                } else {
                ...
                elements.payBtn.textContent = 'Pay & Schedule';
            }
        };
    }
